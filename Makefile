...
.PHONY: flake8
flake8: venv ## flake8
	$(VENV_BIN)/flake8 aioapp_http examples tests setup.py

.PHONY: bandit
bandit: venv  # find common security issues in code
	$(VENV_BIN)/bandit -r ./aioapp_http ./examples setup.py

.PHONY: mypy
mypy: venv ## lint
	$(VENV_BIN)/mypy aioapp_http examples setup.py --ignore-missing-imports
...
coverage-quiet: venv ## make coverage report
		$(VENV_BIN)/coverage run --source aioapp_http -m pytest
...
docs-quiet: venv ## make documentation
	rm -f docs/aioapp_http.rst
	rm -f docs/modules.rst
	.venv/bin/sphinx-apidoc -o docs/ aioapp_http
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
